...
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QPropertyAnimation,  QSettings
...
class ApiKeyManager:
    ...
    def save_api_keys(self):
        self.settings_manager.set("api_keys", self.api_keys)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Beállítások és API kulcsok kezelése
        self.settings_manager = SettingsManager(CONFIG_FILE)
        self.api_key_manager = ApiKeyManager(self.settings_manager)
        self.current_key = self.settings_manager.get("current_key", "")
        ...
        self.api_key_combo.setStyleSheet("""
            ...
            QComboBox:focus, QComboBox:hover, QComboBox:on {
        ...
        def get_openrouter_models():
            ...
            try:
                response = requests.get("https://openrouter.ai/api/v1/models?order=newest&max_price=0")
                ...
                for modelo in models:
                    ...
                    if (any(key in model_id.lower() for key in ["deepseek", "maverick", "google", "openrouter"]) and
                            modelo.get("context_length", 0) >= 128000 and
                            (not self.free_only_checkbox.isChecked() or "free" in model_id.lower())
                        ):
                        is_free = "free" in model_id.lower()
                        context_len = modelo.get("context_length", 0)
                        label = model_id + (" 🆓 " if is_free else "💲") + f" {context_len // 1000} K"
                        filtered_models.append(label)
                        

                return filtered_models
                
            except Exception as e:
                    print("Hiba modellek lekérésekor:", str(e))
                    return []
        ...
        self.free_only_checkbox = QCheckBox(" Ingyenes ")
        self.free_only_checkbox.setChecked(True)
        model_layout.addWidget(self.free_only_checkbox)
        model_layout.addWidget(model_label)
        model_layout.addWidget(self.model_combo)
        
       
        # Lekérjük az openrouter.ai modelleket
        
        openrouter_models = get_openrouter_models()
        self.openr  =  []
        def uplist():
            self.model_combo.clear() 
            openr = list(dict.fromkeys(get_openrouter_models())) 
            self.model_combo.addItems(openr)
             
        self.free_only_checkbox.stateChanged.connect(uplist)    
        ...
        self.tokens_spin.addItems(["1024", "2048", "4096", "8192", "16384", "32000", "64000", "128000"])
        self.tokens_spin.setCurrentIndex(7)
        ...
        """)
        self.response_area.setMaximumBlockCount(10000)
    ...
    def load_settings(self):
        # Load API keys and current key from settings
        api_keys = self.api_key_manager.load_api_keys()
        self.api_key_combo.clear()
        for name in api_keys:
            self.api_key_combo.addItem(name)

        current_key = self.settings_manager.get("current_key", "")
        ...
        # Load other settings
        self.temp_spin.setValue(float(self.settings_manager.get("temperature", 0.4)))
        
        max_tokens = self.settings_manager.get("max_tokens", "128000")
        index = self.tokens_spin.findText(max_tokens)
        if index >= 0:
            self.tokens_spin.setCurrentIndex(index)
        
        free_only = self.settings_manager.get("free_only", True, type=bool)
        self.free_only_checkbox.setChecked(free_only)

    def save_settings(self):
        # Save current key to settings
        current_key = self.api_key_combo.currentText()
        self.settings_manager.set("current_key", current_key)
        
        # Save other settings
        self.settings_manager.set("temperature", self.temp_spin.value())
        self.settings_manager.set("max_tokens", self.tokens_spin.currentText())
        self.settings_manager.set("free_only", self.free_only_checkbox.isChecked())

    def remove_api_key(self, key_name):
        self.api_key_manager.remove_api_key(key_name)
    ...
    def handle_finished(self):
        ...
        if not self.is_continuation:
            # Normál válasz hozzáadása
            self.conversation_history.append({
                "role": "assistant", 
                "content": self.last_response
            })
        else:
            # Folytatott válasz hozzáadása
            # Az utolsó elem a folytatási kérés, ezt cseréljük a válaszra
        ...
        if self.current_history_file:
            success, _ = HistoryManager.save_history(
                self.conversation_history, 
    ...
    def highlight_code_blocks(self):
        ...
        # Regex a kódrészletek megtalálásához
        pattern = r'```([\w\+]*)\n?([\s\S]*?)\n```'
    ...
    def add_api_key(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if key_data['name'] and key_data['key']:
                self.api_key_manager.add_api_key(key_data['name'], key_data['key'])
    ...
    def send_request(self):
        ...
        if not api_key_name or not self.api_key_manager.get_api_key(api_key_name):
            self.response_area.appendPlainText("Kérlek válassz egy érvényes API kulcsot!")
            return

        api_key = self.api_key_manager.get_api_key(api_key_name)
